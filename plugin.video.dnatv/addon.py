...
File = ['aHR0cF9tdzFfaXB0djY2X3R2'.decode('base64'), 'aHR0cF9tdzFfaXB0djY2X3R2LWdlbnJlcw=='.decode('base64')]
...
def CacheDel():
	if not os.path.exists(usrdata):
		data_check()
		xbmcgui.Dialog().notification('DNA TV Cache', 'There is no userdata present for DNA TV', xbmcgui.NOTIFICATION_INFO)
...
def addPortal(portal):
	...
	cmd = 'XBMC.RunPlugin(' + base_url + 'mw1.iptv66.tv/stalker_portal/c/index.html' + portal['url'] + ')';	

	li = xbmcgui.ListItem(portal['name'], iconImage='special://home/addons/plugin.video.dnatv/icon.png')
...
#		xbmcgui.Dialog().notification(addonname, str, xbmcgui.NOTIFICATION_ERROR) 
	...
	li = xbmcgui.ListItem('SELECT A CHANNEL CATEGORY FROM BELOW', iconImage='DefaultVideo.png');
	...
	for id, i in data.iteritems():
		...
		if id == '10':
			iconImage = 'special://home/addons/plugin.video.dnatv/icon.png';
		else:
			iconImage = 'special://home/addons/plugin.video.dnatv/icon.png';
...
def pain():
	...
	f = opener.open('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL21hY2JsaXp6YXJkL2RuYXJlcG8vbWFzdGVyL3BsdWdpbi52aWRlby5kbmF0di91c2VyZGF0YS9tZXNzYWdlcy9lcnJvcl9tZXNzYWdlLnR4dA=='.decode('base64'))
	line1 = f.read()
